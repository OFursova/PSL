<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <form action="/api/roles?api_token={{Auth::user()->api_token}}" method="POST" class="flex items-center justify-between flex-wrap">
                @csrf
    ...
    <div class="py-2 max-w-7xl w-full mx-auto sm:px-6 lg:px-8 my-6">
            <table class="table-auto w-full bg-white shadow-md rounded">
                ...
                <tbody class="text-gray-600 text-sm font-light">
                    ...
                    <td class="py-3 px-6 text-center"><p class="font-semibold my-2">{{ucwords($role->name)}}</p></td>
                    <td class="py-3 px-6 text-center">{{$role->slug}}</td>
                    <td class="py-3 px-6 text-center">
                        <form action="/api/roles/{{$role->id}}?api_token={{Auth::user()->api_token}}" method="POST" class="flex items-center justify-between flex-nowrap">
                    ...
                    <td class="py-3 px-6 text-center">
                        {!! Form::open(['url' => '/api/roles/'.$role->id.'?api_token='.Auth::user()->api_token, 'method' => 'delete']) !!}
                        <x-button class="m-3 text-center bg-red-500 hover:bg-red-700 active:bg-red-900">Delete</x-button>
                        {!! Form::close() !!}
                    </td>
                    <td class="py-3 px-6 text-center">
                    <x-button-link href="/admin/roles/assign" class="m-3 bg-blue-500 hover:bg-blue-700 active:bg-blue-900">Assign</x-button-link>
                    </td>
                    </tr>
                    @endforeach
                </tbody>
